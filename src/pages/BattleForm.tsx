...
const BattleForm = () => {
  ...
  useEffect(() => {
    if (selectedTeam.length > 4) {
      setError("root", { message: "You can only select 4 Pokémon." });
    } else if (selectedTeam.length < 4 && selectedTeam.length > 0) {
      setError("root", { message: "You must select exactly 4 Pokémon." });
    } else {
      clearErrors("root");
    }
  ...
  return (
    <>
      <div className="container bg-[#b8f3fc]">
        <h1 className="text-center title">Welcome to Luna Edge technical interview</h1>
        <form onSubmit={handleSubmit(onSubmit)} className="mx-auto py-[40px]">
          ...
          {errors.root?.message && <p className="my-[30px] text-center text-[#ff0000]">{errors.root.message}</p>}
          ...
          <button type="submit" className="bg-[#8fcc5e] rounded-full text-white p-[10px] font-bold w-auto flex mx-auto mt-[40px]">
            Submit
          </button>
        </form>
      </div>
